...
vignette: >
  %\VignetteIndexEntry{Learning about published polygenic risk score(s)}
...
library(dplyr) # Need this here before library(quincunx) because of n().
```
...
In the cohorts table you can check which cohorts belong to which samples. Note that the unique identification of a sample is given by the combination of the values of the first two columns: `pgs_id`, and `sample_id`.
